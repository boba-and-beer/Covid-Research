...
def displayResults(hit_dictionary, answers, question, displayHTML=False, displayTable=True):
    ...
    htmlDir = os.path.join(workingPath, 'htmlFiles')
    ...
    print("This is pandasData")
    print(pandasData)
    with open(htmlFileName, 'w') as f:
        f.write(all_HTML_txt)
    display(HTML(question_HTML))
    if displayTable:
        df = pd.DataFrame(pandasData, columns = ['Lucene ID', 'BERT-SQuAD Answer', 'Confidence', 'source'])
        display(HTML(df.to_html(render_links=True, escape=False)))

    return pandasData

def searchDatabase(question, keywords, pysearch, lucene_database, BERTSQuAD_Model, displayTable=True, displayHTML=False):
    ...
    print("The hit didctionary is")
    print(len(hit_dictionary.keys()))
